...
def interestingfield():
    ...
    if(request.form.get('selectfield')!=None):
        selected_field = str(request.form.get('selectfield'))
    elif(request.form.get('selectfield')==None):
        selected_field=527355
    ...
    WHERE Scientific_field_ID = '{}' AND GETDATE() < Project$.End_date
    """.format(selected_field)
    rs.execute(queryString)
    ...
    RIGHT JOIN WorksOnProject$ ON WorksOnProject$.Project_ID = Project$.Project_ID
    INNER JOIN ProjectScientificField$ ON Project$.Project_ID = ProjectScientificField$.Project_ID
    WHERE Scientific_field_ID = '{}' AND GETDATE() < Project$.End_date AND Evaluation$.Evaluation_date > DATEADD(year,-1,GETDATE())
    UNION
    ...
    INNER JOIN ProjectScientificField$ ON Project$.Project_ID = ProjectScientificField$.Project_ID
    WHERE Scientific_field_ID = '{}' AND GETDATE() < Project$.End_date
    """.format(selected_field,selected_field,selected_field)
